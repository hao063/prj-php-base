...
<?php
// truy vấn đơn hàng giao hàng thành công
    $id_user = $_SESSION['customer']['id'];
    $sql = "SELECT * FROM `order` WHERE id_user = '$id_user' and status = 5";
    $result = db_query($sql);
?>
<!-- view đơn hàng -->
<div class="container_all_or">
    <!-- duệt tất cả đơn hàng -->
        ...
        <div class="contain_all">
            ...
            <div>
                <div class="title">
                    ...
                    <a href="?page=customer_order&page_or=delete_user_or&action=delete_accept&id=<?=$value['id']?>"><i class="fas fa-trash-alt delete_order_user"></i></a>
                </div>
            </div>
        </div>
            <!-- duyệt chi tiết các sản phẩm của một đơn hàng -->

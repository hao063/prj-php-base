<!-- 
    ...
    -->
<?php
// truy vấn ra những order phản hồi từ khách hàng
    $id_user = $_SESSION['customer']['id'];
    $sql = "SELECT * FROM `order` WHERE id_user = '$id_user' and status = 3";
    $result = db_query($sql);
?>
<!-- view order -->
<div class="container_all_or">
    ...
    <div class="footer">
            ...
            <form action="?page=customer_order&page_or=sub_continue" method="post">
                <input type="hidden" name="cancel_order" value="<?= $value['id'] ?>">
                <input type="submit" name="submit_cancel" value="Tiếp tục đặt">
            </form>
        </div>
        </div>
    <?php } ?>
